...
add_filter( 'wpmem_email_newreg', 'add_favorite_posts_to_reg_mail' );
function add_favorite_posts_to_reg_mail( $email_content )
{
    $favorite_links = "";
    ...
    if (wpfp_get_cookie()):
        foreach (wpfp_get_cookie() as $post_id => $post_title) {
            $p = get_post($post_id);
            $favorite_links .= $p->post_title.': '.get_permalink($post_id)."\n";
        }
    endif;

    $search_tags      = array("[**wp-favorite-posts**]");
    $replace_values   = array($favorite_links);

    return str_replace($search_tags,$replace_values,$email_content);
}